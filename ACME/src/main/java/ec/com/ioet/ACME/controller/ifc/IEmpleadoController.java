...
public interface IEmpleadoController {
	...
	default ResponseEntity<?> consultaMarcacionbyFecha(@ApiParam(value = "",required=true) @PathVariable("fechaInicio") String fechaInicio,@ApiParam(value = "",required=false) @PathVariable("fechaFin") String fechaFin)throws ParseException{
		getRequest().ifPresent(request -> {
            for (MediaType mediaType: MediaType.parseMediaTypes(request.getHeader("Accept"))) {
                if (mediaType.isCompatibleWith(MediaType.valueOf("application/json"))) {
                    String exampleString = "[\r\n" + 
